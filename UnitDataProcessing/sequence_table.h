...
namespace structures
{
	...
	inline T SequenceTable<K, T>::remove(const K& key)
	{
		...
		if (item != nullptr) {
			...
			list_->tryRemove(item); //list_->tryRemove(data);
			delete item;
			return data;
		}
		else {
	...
	{
		return findTableItem(key) != nullptr; // ? true : false
	}
	...
	inline SequenceTable<K, T>& SequenceTable<K, T>::assignSequenceTable(SequenceTable<K, T>& other)
	{
		if (this != &other) {
			clear();
			for (auto item : *other.list_) { //other by fungovalo tiež namiesto *other.list_
