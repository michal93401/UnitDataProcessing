...
namespace structures
{
	...
	inline void SortedSequenceTable<K, T>::insert(const K& key, const T& data)
	{
		...
		bool yes = false;
		int index = indexOfKey(key, 0, this->size() - 1, yes);
	...
	inline TableItem<K, T>* SortedSequenceTable<K, T>::findTableItem(const K& key)
	{
		bool yes = false;
		int index = indexOfKey(key, 0, this->size() - 1, yes);
		return yes ? SequenceTable<K, T>::list_->at(index) : nullptr;
	}
		...
		if (key == keyMiddle) {
			...
			if (indexStart == indexEnd) {
				...
				if (keyMiddle < key) {
					...
					return indexOfKey(key, indexStart, indexMiddle, found); //lebo sebik povedal
				}
			}
		}
	}
